...
| Layer | Name             | Data Unit | Medium / Role                         | Addressing / Identifier    | Typical Devices                 | Example Protocols / Technologies       |
| ----- | ---------------- | --------- | ------------------------------------- | -------------------------- | ------------------------------- | -------------------------------------- |
| 7     | **Application**  | Data      | User interface / network applications | Application name / URL     | Computers, Phones, Web Browsers | HTTP, FTP, DNS, SSH              |
| 6     | **Presentation** | Data      | Encoding / encryption / compression   | File types / Encoding info | Codecs, SSL Libraries           | SSL/TLS, JPEG, MPEG, ASCII             |
| 5     | **Session**      | Data      | Connection management (start/stop)    | Session ID                 | Application Servers, Gateways   | NetBIOS, RPC, SMB session              |
| 4     | **Transport**    | Segments  | Reliable delivery, flow control       | **Port number**            | Firewalls, Load Balancers       | TCP, UDP                         |
| 3     | **Network**      | Packets   | Routing between networks              | **IP address**             | Routers, Layer 3 Switches       | IP (IPv4/6), ICMP         |
| 2     | **Data Link**    | Frames    | Local delivery on a single link       | **MAC address**            | Switches, NICs (Network Interface Cards), Bridges         | Ethernet, ARP, VLAN (802.1Q)      |
| 1     | **Physical**     | Bits      | Physical bit transmission             | Physical medium (signal)   | Cables, Hubs, Repeaters         | RJ45, Fiber, Bluetooth, Wi-Fi (802.11) |
...
| -------------------------- | ------ | ---------------------- | --------------------- | ----------------------------------- |
| **Normal ARP Request**     | `1`    | ❌                     | ✅ or ❌ (can be **Unicast** if ARP cache is assumed stale to reduce network traffic) | Resolving MAC from IP (*Who has IP `x.x.x.x`?* - Expects **ARP Reply**) |
| **Normal ARP Reply**       | `2`    | ❌                     | ❌ (**Unicast**) | Answering ARP request (*I have IP `x.x.x.x`, my MAC is `yy:yy:yy:yy:yy:yy`*) |
| **Gratuitous ARP Request** | `1`    | ✅                     | ✅           | IP conflict detection, announcement (*Who has my IP `x.x.x.x`?* - If no one responds, the device assumes it owns that IP safely - a response is usually a **Gratuitous ARP Reply** by the device who has the IP `x.x.x.x`) |
| **Gratuitous ARP Reply**   | `2`    | ✅                     | ✅ or ❌ (can be **Unicast** to reduce network traffic by notifying a specific device) | IP takeover, update ARP caches in network (*My MAC is `yy:yy:yy:yy:yy:yy` and my new IP is `x.x.x.x`, update your ARP cache* - usually sent after a **Gratuitous ARP Request**) |
...
*Why is MTU Discovery needed?*

Different links between two endpoints might have different MTU sizes.
Sending packets larger than the smallest MTU along the path causes fragmentation by layer 3 switches/routers (in IPv4) or dropped packets (in IPv6).
Fragmentation can heavily degrade performance and cause delays.

- The sender starts sending packets with the MTU size of its own interface (often 1500 bytes)
- If a router along the path has a smaller MTU and cannot forward the packet without fragmentation:
  - In IPv4, if the packet has the "Don't Fragment" (DF) flag set, the router drops the packet
  - The router then sends back an ICMP "Fragmentation Needed" message to the sender, indicating the MTU of the next hop.
- The sender reduces its packet size to the MTU indicated in the ICMP message and retries.
- This process repeats until the sender finds the smallest MTU on the path.
